...
async function main() {
  ...
  app.listen(3000, () => {
    ...
    console.log('📦 curl -X POST http://localhost:3000/test -H \"Content-Type: application/json\" -d \'{\"mensaje\": \"API Test\"}\'');
  });
}
...
echo "🚀 Ejecutando IntegridAI API..."
node --experimental-vm-modules api-server.js
